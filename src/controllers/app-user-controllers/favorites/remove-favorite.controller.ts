  ...
  try {
    ...
    if (!user.favorites.includes(recommendation._id)) {
      return res.status(400).json({
        message: "Recommendation is not in favorites",
        errorCode: ErrorCode.NOT_FAVORITED
      })
    }

    // Remove from favorites using $pull
    await AppUser.findByIdAndUpdate(userId, {
      $pull: { favorites: recommendation._id }
    })
    ...
    return res.status(200).json({
      message: "Recommendation removed from favorites successfully",
      data: updatedUser
    })
  } catch (error) {
